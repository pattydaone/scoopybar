...
void
strip_whitespace(char *buf, int index)
{
    int front_offset = 0;
    ...
    while (buf[front_offset] == ' ')
        ++front_offset;
    memmove(buf, buf + front_offset, (index - front_offset + 1) * sizeof(char));
}

struct ConfParser *
PARSER_create(const char *file_path, int buf_sz)
{
...
enum PARSER_CODES
PARSER_find(struct ConfParser *p, const char *key, char *des)
{
...
enum PARSER_CODES
PARSER_next_section(struct ConfParser *p)
{
    int cur = 0;
...
enum PARSER_CODES
PARSER_next_kv(struct ConfParser *p)
{
    int cur = 0;
...
int
PARSER_clean(struct ConfParser *p)
{
