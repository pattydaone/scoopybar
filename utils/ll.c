...
void
LL_push_back_event(struct event_node **head, struct event *data)
{
    ...
    struct event_node *current = *head;
    while (current->next != NULL) {
        current = current->next;
    }
...
void
LL_delete_event(struct event_node **head, struct event_node *data)
{
    if (*head == data) {
        *head = data->next;
        free(data);
